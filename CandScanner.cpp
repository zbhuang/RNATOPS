...
Candid * CandScanner::scanTopnInOneRegionSP( Stem *pstem, CYKSearch &curcyk, double coeff, int runned )
{
    ...
    {
        offset[0]=pstem->getLeftOffset( coeff, 0 );
        offset[1]=pstem->getRightOffset( coeff, 1 );
...
Candid * CandScanner::scanTopnInTwoRegion( Stem *pstem,  CYKSearch & curcyk, double coeff )
{
    #ifdef DEBUG_DEV
        printf("CandScanner::scanTopnInTwoRegion( Stem )\n");
    #endif
    int i=0, j=0, numreg=0, scanlen=0;
    int offset[2]={0, 0};
    ...
    //**** beg ****
    int leftArmReg[2]={0,0};
    int rightArmReg[2]={0,0};
    int midreg[2]={0,0};
    numreg = pstem->getNumPairRegion(  );
    for( j=0; j<numreg; j++ )
    {
        pstem->getLeftArmRegion( coeff, leftArmReg, j );
        offset[0]=leftArmReg[0];
        midreg[0]=leftArmReg[1]<=seqlen-1 ? leftArmReg[1] : seqlen-1;
        if( offset[0]>seqlen-2 )
            continue;

        pstem->getRightArmRegion( coeff, rightArmReg, j );
        midreg[1]=rightArmReg[0];
        offset[1]=rightArmReg[1]<=seqlen-1 ? rightArmReg[1] : seqlen-1;
        if( midreg[1]>seqlen-1 )
            continue;
        /*printf("Stem %c, region %d, [%d-%d][%d-%d]\n", pstem->idchar, j, offset[0], midreg[0], midreg[1], offset[1]);
        for(i=offset[0]; i<=midreg[0]; i++ )
            printf("%c", numtochar(searchSeq[i]) );
        printf("   ");
        for(i=midreg[1]; i<=offset[1]; i++ )
            printf("%c", numtochar(searchSeq[i]) );
        printf("\n");*/
        if( allowOffset )  //disable or enable Offset
        {
        ...
        curcyk.CYKSearchTwoRegion( seqseg, scanlen, midreg, pstem, j );
        //**** end ****
...
int CandCollect::searchCandids( Stem *pstem, int numsm,  int topn,  double thres,  char * pStr)
{
    #ifdef DEBUG_DEV
    ...
    for( i=0; i<numsm; i++ )
    {
        ...
        if( bTwoRegion ) //two-region method, but couldn't speedup
            pCD=scan.scanTopnInTwoRegion( &pstem[i], cyks[i], coefStDev );
