...
void CYKSearch::setBoundaryConds( Stem *curstem, int sequlen )
{
...
int CYKSearch::CYKSearchString( int *instr, int newlen, Stem *curstem, int haverun, int shiftnum )
{
    ...
    for(i=0; i<2; i++ )
    {
        offsetE[i]=curstem->steminfo.offsetE[i][0];
        offsetSD[i]=curstem->steminfo.offsetSD[i][0];
        armE[i]=curstem->steminfo.armE[i];
        armSD[i]=curstem->steminfo.armSD[i];
    }
    midE = curstem->steminfo.midE;
    midSD = curstem->steminfo.midSD;
    ...
    update3DandSeq( newlen, newnumntm );
    //2.copy searchSeq
    for( i=0; i<seqlen; i++ )
    {
         searchSeq[i] = instr[i];
...
int CYKSearch::CYKSearchTwoRegion( int *instr, int newlen, int midreg[2], Stem *curstem, int regNo )
{
    ...
    for(i=0; i<2; i++ )
    {
        offsetE[i]=curstem->steminfo.offsetE[i][ regNo ];
        offsetSD[i]=curstem->steminfo.offsetSD[i][ regNo ];
        armE[i]=curstem->steminfo.armE[i];
        armSD[i]=curstem->steminfo.armSD[i];
    }
    midE = curstem->steminfo.midE;
