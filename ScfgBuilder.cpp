...
int Stem::getStemSpan( double coeff, int ismax )
{
    ...
    double sum=0.0;
    if( ismax )
        sum = steminfo.armE[0] + steminfo.armE[1] + steminfo.midE + 
              coeff*(steminfo.armSD[0] + steminfo.armSD[1] + steminfo.midSD);
    else
        sum = steminfo.armE[0] + steminfo.armE[1] + steminfo.midE - 
              coeff*(steminfo.armSD[0] + steminfo.armSD[1] + steminfo.midSD);
    sum = sum>0 ? sum : 0;
...
int Stem::getLeftOffset( double coeff, int ismax, int thd )
{
    int retval=0;
    if( ismax )
        retval = (int)( steminfo.offsetE[0][thd] + coeff * steminfo.offsetSD[0][thd]);
    else
        retval = (int)( steminfo.offsetE[0][thd] - coeff * steminfo.offsetSD[0][thd]);
...
int Stem::getRightOffset( double coeff, int ismax, int thd )
{
    int retval=0;
    if( ismax )
        retval = (int)( steminfo.offsetE[1][thd] + steminfo.armE[1] + 
                        coeff*(steminfo.offsetSD[1][thd] + steminfo.armSD[1]) );
    else
        retval = (int)( steminfo.offsetE[1][thd] - coeff * steminfo.offsetSD[1][thd] );
    retval = retval>0 ? retval : 0;
    return retval;
}
...
{
    return steminfo.numOffset;
}

int Stem::getLeftArmRegion( double coeff, int region[2], int thd )
{
    region[0] = (int)( steminfo.offsetE[0][thd] - coeff*steminfo.offsetSD[0][thd]);
    region[0] = region[0]>0 ? region[0] : 0;
    region[1] = (int)( steminfo.offsetE[0][thd] + steminfo.armE[0] - 1 +
                       coeff*(steminfo.offsetSD[0][thd] + steminfo.armSD[0]) );
    //printf("getLeftArmRegion: %d - %d \n",  region[0], region[1]);
    return 1;
}

int Stem::getRightArmRegion( double coeff, int region[2], int thd )
{
    region[0] = (int)( steminfo.offsetE[1][thd] - coeff*steminfo.offsetSD[1][thd]);
    region[0] = region[0]>0 ? region[0] : 0;
    region[1] = (int)( steminfo.offsetE[1][thd] + steminfo.armE[1]  - 1 +
                       coeff*(steminfo.offsetSD[1][thd] + steminfo.armSD[1]) );
    //printf("getRightArmRegion: %d - %d \n",  region[0], region[1]);
    return 1;
}

int Stem::getLeftArmlen( double coeff, int ismax )
{
    int retval=0;
    if( ismax )
        retval = (int)( steminfo.armE[0]+ coeff*steminfo.armSD[0]);
    else
        retval = (int)( steminfo.armE[0]- coeff*steminfo.armSD[0]);
    retval = retval>0 ? retval : 0;
    return retval;
...
int Stem::getRightArmlen( double coeff, int ismax )
{
    int retval=0;
    if( ismax )
        retval = (int)( steminfo.armE[1]+ coeff*steminfo.armSD[1]);
    else
        retval = (int)( steminfo.armE[1]- coeff*steminfo.armSD[1]);
    retval = retval>0 ? retval : 0;
    return retval;
...
int Stem::getMidlen( double coeff, int ismax )
{
    int retval=0;
    if( ismax )
        retval=(int)( steminfo.midE + coeff*steminfo.midSD );
    else
        retval=(int)( steminfo.midE - coeff*steminfo.midSD );
    retval = retval>0 ? retval : 0;
    return  retval;
...
void ScfgBuilder::seperateStem(  )
{
    ...
    int seqsnum=allstems[0].numseqs;
    ...
    for( i=0; i<numstems; i++ )
    {
        ...
        allstems[i].armseq[0]= new int*[ seqsnum ];
        allstems[i].armseq[1]= new int*[ seqsnum ];
        for( j=0; j<seqsnum; j++ )
        {
            allstems[i].armseq[0][j]= new int[ lenl ];
            for(k=0; k<lenl; k++ )
            {
                allstems[i].armseq[0][j][k]=sequences[j][ls+k];
            }
            
            allstems[i].armseq[1][j]= new int[ lenr ];
            for(k=0; k<lenr; k++ )
            {
                allstems[i].armseq[1][j][k]=sequences[j][rs+k];
            }
        }
    }
}
...
char ScfgBuilder::determPairState(Stem *curstem, int lidx, int ridx )
{
    ...
    #ifdef DEBUG_DEV
        printf("   BasePair: %d, GapPair: %d in %d seqs.\n", nongappair, numseqs-nongappair, numseqs);
    #endif
    if( nongappair>(numseqs-nongappair))
        return 'S';
    else
        return 'I';
}
                               ...
                               int begl,  int begr,   int distl,  int distr )
{
    ...
    //S-->S
    /*prob[SCFG_MS]= (double)(cntss+pdocnt)/(numseqs+2*pdocnt);
    tempprob=(1-prob[SCFG_MS])*(cntsl+pdocnt)/( cntsr + cntsl + 2*pdocnt );
    if( bulge[0]>0 )
    {
        prob[SCFG_LFI]= tempprob*(cntslh+pdocnt)/(numseqs+2*pdocnt);
        prob[SCFG_LFH]= tempprob*(numseqs-cntslh+pdocnt)/(numseqs+2*pdocnt);
    }
    else
    {
    ...
    if( bulge[1]>0 )
    {
        prob[SCFG_RGI]= tempprob*(cntsrh+pdocnt)/(numseqs+2*pdocnt);
        prob[SCFG_RGH]= tempprob*(numseqs-cntsrh+pdocnt)/(numseqs+2*pdocnt);
    }
    else
    {
    ...
    */
    ...
    if( bulge[0]>0 )
    {
        pSG->trprob[SCFG_MS]= (double)pdocnt/(numseqs+3*pdocnt);
        pSG->trprob[SCFG_LFI]= (double)(cntslh+pdocnt)/(numseqs+3*pdocnt);
        pSG->trprob[SCFG_LFH]= (double)(numseqs-cntslh+pdocnt)/(numseqs+3*pdocnt);
    }
    else
    {
        pSG->trprob[SCFG_MS]= (double)(numseqs-cntsl+pdocnt)/(numseqs+2*pdocnt);
        pSG->trprob[SCFG_LFI]= (double)(cntsl+pdocnt)/(numseqs+2*pdocnt);
        pSG->trprob[SCFG_LFH]= 0.0;
    }
    //V-->S
    if( bulge[1]>0 )
    {
        pVT->trprob[SCFG_MS]= (double)pdocnt/(numseqs+3*pdocnt);
        pVT->trprob[SCFG_RGI]= (double)(cntsrh+pdocnt)/(numseqs+3*pdocnt);
        pVT->trprob[SCFG_RGH]= (double)(numseqs-cntsrh+pdocnt)/(numseqs+3*pdocnt);
    }
    else
    {
        pVT->trprob[SCFG_MS]= (double)(numseqs-cntsr+pdocnt)/(numseqs+2*pdocnt);
        pVT->trprob[SCFG_RGI]= (double)(cntsr+pdocnt)/(numseqs+2*pdocnt);
...
void ScfgBuilder::calcPairEmProb( ScfgGram *pSG,  Stem *pstem,  int lpos,  int rpos)
{
    ...
    for( i=0; i<5; i++ )
    {
        for( j=0; j<5; j++ )
        {
            pSG->emprobS[i][j]=(pair[i][j]+pdocnt)/(numseqs+25*pdocnt);
        }
    }
    */
    for( i=0; i<5; i++ )
    {
        ...
        {
            if( i==0 || j==0 )
                pSG->emprobS[i][j]=(pair[i][j]+pdocntgap)/(numseqs + 16*pdocnt + 9*pdocntgap);
            else
                pSG->emprobS[i][j]=(pair[i][j]+pdocnt)/(numseqs + 16*pdocnt + 9*pdocntgap);
        }
    }
}
...
void ScfgBuilder::calcLoopTransPLeft(  Stem *pstem, int beg, int dist, double &lpi, double &lplp )
{
    ...
    ss=numseqs-si;
    lpi=(si+pdocnt)/(numseqs+2*pdocnt);
    lplp=(ss+pdocnt)/(numseqs+2*pdocnt);
}
...
int ScfgBuilder::setTrainingData( DataLoader &trdata )
{
    ...
    for(i=0; i<numstems; i++)
    {
        ...
        allstems[i].steminfo=trdata.stemary[i];
...
void ScfgBuilder::countOverallPairFreq(  )
{
    #ifdef DEBUG_DEV
        printf("ScfgBuilder::countOverallPairFreq\n");
    #endif
    int i=0, j=0, k=0;
    for(i=0; i<numstems; i++ )
    {
